...
export function calcularRegistroImoveis(valorImovel: number): number {
  if (!valorImovel || valorImovel <= 0) return FAIXAS_REGISTRO_IMOVEIS[0].total;
  for (const f of FAIXAS_REGISTRO_IMOVEIS) {
    if (valorImovel <= f.ate) return f.total;
  }
  const ultima = FAIXAS_REGISTRO_IMOVEIS[FAIXAS_REGISTRO_IMOVEIS.length - 1];
  const excedente = Math.ceil((valorImovel - ultima.ate) / 50000);
  return ultima.total + excedente * 63.87;
}
